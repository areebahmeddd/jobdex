...
5. **Hydrate**: `hydrate()` runs over the new postings only, so per-job detail requests scale with what changed, not board size.
...
| BambooHR              | Global       | `{slug}.bamboohr.com/careers/list` no longer returns JSON. Unknown tenants get the same 43,538-byte marketing page as known ones (verified against a slug chosen not to exist); tenants that do have a board serve an HTML SPA                           |
...
Every platform previously listed here has been probed live. Failures are recorded in the Not Compatible table, SEEK in the Planned table. Probe scripts and raw responses live under `ats/`, which is gitignored, matching the earlier per-region research. Two platforms are deferred, not rejected, because the endpoint works and only the listings are missing:

- **MCF** (Singapore): integrated and working, but no companies are registered. Its board is national, not per-company, so the 96k listings sit behind thousands of employers and the top posters are staffing agencies (SSIC `78xxx`), not employers. Registering them as individual crawl targets would push the crawl rotation past a day. Companies are addressable exactly by `?uen=`; `?company=` is a substring match and over-matches. Left unseeded on purpose.
